...
public class DatabaseHandler extends SQLiteOpenHelper {
    ...
    private static final String NUMBER_OF_PLANS = "no_of_plans";
    private static final String MONTHLY_PRICE = "monthly_price";
    private static final String MONTHLY_SELECT = "n";
    ...
    public boolean setCart(HashMap<String, String> map, Float Qty) {
        ...
        if (isInCart(map.get(COLUMN_ID))) {
            ...
            values.put(NUMBER_OF_PLANS, map.get(NUMBER_OF_PLANS));
            values.put(MONTHLY_PRICE, map.get(MONTHLY_PRICE));
    ...
    public void update_no_flpan(int stat, String p_id)
    {
        db = getWritableDatabase();

        if (stat == 0 || stat == 1 || stat == 2)
        {
            db.execSQL("update " + CART_TABLE + " set " + NUMBER_OF_PLANS + " = " + stat + " where " + COLUMN_ID + " = " + p_id );
    ...
    public double gettTotalAmount() {
        ...
        if (cursor!=null) {
                ...
                for (int i = 0; i < cursor.getCount(); i++) {
                    ...
                    if (!cursor.getString(cursor.getColumnIndex("service")).equals("0")) {
                        p_total = p_total + (cursor.getInt(cursor.getColumnIndex("qty")) * cursor.getDouble(cursor.getColumnIndex("monthly_price")));
                        ...
                        Log.e("DB ELSE TOTAL : 0", " " + p_total + " for product "+
                                cursor.getString(cursor.getColumnIndex("product_name")));
                        cursor.moveToNext();
                    }
...
//                    }
    ...
    public ArrayList<HashMap<String, String>> getCartAll() {
        ...
        for (int i = 0; i < cursor.getCount(); i++) {
            ...
            map.put(SERVICE,cursor.getString(cursor.getColumnIndex(SERVICE)));
            map.put(NUMBER_OF_PLANS,cursor.getString(cursor.getColumnIndex(NUMBER_OF_PLANS)));
            map.put(MONTHLY_PRICE,cursor.getString(cursor.getColumnIndex(MONTHLY_PRICE)));
